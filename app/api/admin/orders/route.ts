...
export async function POST(request: Request) {
  ...
  const {
    ...
    deliveryMethod,
    paymentMethod,
    email,        // email, переданный клиентом
    name,
  } = body
  ...
  try {
    ...
    const order = await prisma.order.create({
      data: {
        ...
        userId: userId || undefined,   // ← исправлено: undefined вместо null
    ...
    // Очищаем корзину только для авторизованных
    if (userId) {
      await prisma.cartItem.deleteMany({ where: { userId } })
    }

    // Определяем email покупателя
