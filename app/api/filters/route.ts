  ...
  try {
    if (includeCounts) {
      ...
      allCategories.forEach(cat => {
        categoryCounts[cat.slug] = allProducts.filter(p => p.category.slug === cat.slug).length
      })

      const tagCounts: Record<string, number> = {}
      allProducts.forEach(p => {
        const tags = extractUnique([p.tags])
        tags.forEach(tag => { tagCounts[tag] = (tagCounts[tag] || 0) + 1 })
      })

      const filterCounts: Record<string, Record<string, number>> = {}
      for (let i = 1; i <= 15; i++) {
        filterCounts[`filter${i}`] = {}
      }
      filterCounts['scales'] = {}

      for (const p of allProducts) {
        for (let i = 1; i <= 15; i++) {
          const field = `filter${i}` as keyof typeof p
          const vals = extractUnique([(p as any)[field] || ''])
          vals.forEach(v => {
            filterCounts[field][v] = (filterCounts[field][v] || 0) + 1
          })
        }
        const scaleArr = extractUnique([p.scale || ''])
        scaleArr.forEach(v => {
          filterCounts['scales'][v] = (filterCounts['scales'][v] || 0) + 1
        })
      }

      return NextResponse.json({ categoryCounts, tagCounts, filterCounts })
    }
