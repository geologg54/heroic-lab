...
import { Plus, Package, ShoppingCart, Users, FileText, MessageCircle } from 'lucide-react'
...
export const QuickActions = () => (
  <div className="mt-6 bg-cardbg border border-borderLight rounded-xl p-4">
    ...
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
      {actions.map(action => (
        <Link key={action.name} href={action.href} className={`${action.color} hover:opacity-80 rounded-lg p-3 text-center transition flex flex-col items-center gap-2 text-white`}>
          <action.icon size={20} />
