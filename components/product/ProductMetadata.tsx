...
export const ProductMetadata = ({ product }: { product: Product }) => (
  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8 p-4 bg-cardbg rounded-xl border border-borderLight">
    ...
    <div>
      ...
      <div className="text-white font-medium">{product.fileFormat}</div>
    </div>
    <div>
      <div className="text-gray-400 text-sm">Файлов в комплекте</div>
      <div className="text-white font-medium">1-5 STL</div>
    </div>
    {/* Поле "Скачиваний" удалено */}
    <div>
      <div className="text-gray-400 text-sm">Поддержка</div>
      <div className="text-white font-medium">Есть</div>
    </div>
  </div>
)